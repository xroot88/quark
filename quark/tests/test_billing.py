...
class QuarkBillingPayloadTest(QuarkBillingBaseTest):
    ...
    def test_fixed_payload(self):
        ...
        payload = billing.build_payload(ipaddress, 'ip.exists', start_time,
                                        end_time)
        ...
        self.assertEqual(payload['startTime'],
                         billing.convert_timestamp(start_time), 'startTime is wrong')
        self.assertEqual(payload['endTime'],
                         billing.convert_timestamp(end_time), 'endTime is wrong')
    ...
    def test_associate_flip_payload(self):
        ...
        payload = billing.build_payload(ipaddress, 'ip.associate', start_time,
                                        end_time)
        #sys.stderr.write('payload = {}'.format(payload))
        ...
        self.assertEqual(payload['eventTime'],
                         billing.convert_timestamp(start_time), 'eventTime is wrong')
    ...
    def test_disassociate_flip_payload(self):
        ...
        ipaddress.deallocated_at = end_time
        ipaddress.address_type = 'floating'
        payload = billing.build_payload(ipaddress, 'ip.disassociate', start_time,
                                        end_time)
        #sys.stderr.write('payload = {}'.format(payload))
        ...
        self.assertEqual(payload['id'], IP_ID, 'ip_id is wrong')
        self.assertEqual(payload['eventTime'],
                         billing.convert_timestamp(end_time), 'eventTime is wrong')
